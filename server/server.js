...
app.get("/cartItemCount", authenticateToken, (req, res) => {
    const userId = req.user.id;
    db.get("SELECT COUNT(*) AS cartItemCount FROM cart_items WHERE user_id = ?", [userId], (err, row) => {
        if (err) {
            res.status(500).send(err.message);
            return;
        }
        res.json({ cartItemCount: row.cartItemCount });
    });
});
